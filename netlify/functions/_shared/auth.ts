...
export async function requireUser(event: any) {
  ...
  const { data: user, error } = await supabase.from('app_users').select('id,auth_provider,telegram_id,telegram_username,google_email,display_name,avatar_url,telegram_chat_id,telegram_notifications_enabled,notification_language,created_at,last_login_at').eq('id', session.userId).single();
